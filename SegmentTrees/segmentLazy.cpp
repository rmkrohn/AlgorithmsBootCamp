...
//Demo program for lazy segment trees
...
int query(int node, int start, int end, int l, int r)
{
    ...
    //node has pending update, do it
	if(lazy[node] != 0)
    {
    	//update node by number of descendant leaf nodes * update value
		tree[node] += (end - start + 1) * lazy[node];
		
		//if not leaf node, mark children as requiring update
		if(start != end)
		{
			lazy[2*node] += lazy[node];     //left
			lazy[2*node+1] += lazy[node];	//right
		}
		lazy[node] = 0;     //mark node as no longer requiring update
    } 
...
void updateRange(int node, int start, int end, int l, int r, int diff)
{
	...
	//node range not within update range, return
	if(start > end || start > r || end < l)
		return;
		
	//node range entirely within update range, update node and mark children
	if(start >= l && end <= r)
	{
		tree[node] += (end - start + 1) * diff;		//update node
		
		//mark children as requiring update
		if(start != end)
		{
			lazy[2*node] += diff;		//left
			lazy[2*node+1] += diff;		//right
		}
		return;
	}
	...
	int mid = (start + end) / 2;
	updateRange(2*node, start, mid, l, r, diff);        //left child
	updateRange(2*node+1, mid+1, end, l, r, diff);  //right child
	
	//set current node value to be sum of children
	tree[node] = tree[2*node] + tree[2*node+1];        
}
